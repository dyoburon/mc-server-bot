...
export class RoleManager {
  ...
  setOverride(botName: string, reason: string, commandId: string): void {
    ...
    logger.info({ botName, reason, commandId }, 'Role override set');
  }

  clearOverride(botName: string): void {
    if (this.overrides.delete(botName)) {
      this.io.emit(FLEET_EVENTS.ROLE_UPDATED, { assignments: this.assignments, overrides: this.getOverrides() });
      logger.info({ botName }, 'Role override cleared');
    }
  }
  ...
  checkOverrideTimeouts(): void {
    ...
    for (const [botName, record] of this.overrides) {
      if (now - record.at > OVERRIDE_EXPIRY_MS) {
        ...
        logger.info({ botName, ageMs: now - record.at }, 'Role override expired');
        changed = true;
      }
    }
    if (changed) {
  ...
  private load(): void {
    try {
      if (fs.existsSync(this.filePath)) {
        ...
        logger.info({ count: this.assignments.length }, 'Role assignments loaded');
      }
    } catch (err) {
      logger.warn({ err }, 'Failed to load role assignments');
      this.assignments = [];
    }
  }
  ...
  private save(): void {
    try {
      ...
      logger.error({ err }, 'Failed to save role assignments');
    }
  }
  ...
  createAssignment(data: {
    ...
    this.save();
    this.emit();
    logger.info({ id: record.id, botName: record.botName, role: record.role }, 'Role assignment created');
    return record;
  }
  ...
  updateAssignment(id: string, data: Partial<RoleAssignmentRecord>): RoleAssignmentRecord | null {
    ...
    this.save();
    this.emit();
    logger.info({ id, updates: Object.keys(updateFields) }, 'Role assignment updated');
  ...
  deleteAssignment(id: string): boolean {
    ...
    this.save();
    this.emit();
    logger.info({ id, botName: removed.botName }, 'Role assignment deleted');
    return true;
  }
}
