...
export function createAPIServer(botManager: BotManager): APIServerResult {
  ...
  // Mission manager — create early so /api/status can reference it
  ...
  app.get('/api/status', (_req: Request, res: Response) => {
    res.json({
      ...
      controlHealth: {
        pendingCommands: commandCenter.getCountByStatus('queued'),
        activeCommands: commandCenter.getCountByStatus('started'),
        failedCommandsLast5min: commandCenter.getRecentFailedCount(5 * 60 * 1000),
        runningMissions: missionManager.getRunningCount(),
        staleMissions: missionManager.getStaleCount(),
      },
    });
  });
  ...
  app.get('/api/metrics', (_req: Request, res: Response) => {
    res.json({
      commands: commandCenter.getMetrics(),
      missions: missionManager.getMetrics(),
    });
  });
